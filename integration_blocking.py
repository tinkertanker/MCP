...
sys.path.append("")
from anim_player import AnimPlayer
...
time_last_pose_detected = time.time()
time_last_idle_frame_played = time.time()
...
while not graceful_killer.kill_now:
    ...
    if pose_state == "stand":
        if time.time() - time_last_pose_detected > idle_timeout:
            if time.time() - time_last_idle_frame_played > frame_period:
                anim_player.step_idle()
                time_last_idle_frame_played = time.time()
                if renderer:
                    cv2.putText(frame, "IDLE", (frame.shape[1] // 2, 100), cv2.FONT_HERSHEY_PLAIN, 3, (0, 190, 255), 3)
